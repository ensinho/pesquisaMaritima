...
import { useColetasByUser, useDeleteColeta } from '@/hooks/useColetas';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { ArrowLeft, Search, Fish, Edit, Trash2, Plus } from 'lucide-react';
...
export default function MinhasColetas() {
  ...
  const { data: coletas, isLoading } = useColetasByUser(userId);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
              ...
              <Card key={coleta.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <CardContent>
                  <div className="space-y-2 text-sm">
                    ...
                    {coleta.observacoes && (
                      <p><strong>Observações:</strong> {coleta.observacoes.substring(0, 100)}{coleta.observacoes.length > 100 ? '...' : ''}</p>
                    )}
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-4">Você ainda não possui coletas cadastradas</p>
            <Button onClick={() => navigate('/nova-coleta')}>
              <Plus className="mr-2 h-4 w-4" />
              Criar sua primeira coleta
            </Button>
          </div>
        )}
        ...
        {deletingColeta && (
          <ConfirmDelete
            ...
            description={`Tem certeza que deseja excluir a coleta "${deletingColeta.nome_comum || deletingColeta.nome_cientifico || 'sem nome'}"? Esta ação não pode ser desfeita.`}
